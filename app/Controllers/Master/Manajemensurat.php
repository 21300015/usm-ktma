...
class Manajemensurat extends BaseController
{
	public $suburl = 'master/manajemensurat';
	public $masksalt = '4m29';
	function __construct() {
		parent::__construct();
		
		$this->modulename = 'Manajemen Surat';
	...
	public function index()
    {
		...
		$data = array();
		$this->addJs('modules/master/manajemensurat.js');
        return $this->ViewDefault($this->suburl.'/pagebody', $data);
    }
    private function grid() {
		$innerapp = $this->innerapp; 
		$filter = "";
		$searchcolumns = array( 'id' => 'a.id', 'jns_surat' => 'jns_surat','perihal' => 'a.perihal');
		$displaycolumns = array('id','jns_surat','perihal');
		$sql = "SELECT a.id, a.no_surat, a.jns_surat, c.nm_jnssurat, a.perihal, a.tglsurat, a.desksurat, a.id_files,
			date_format(a.tglsurat, '%d-%m-%Y') cd, b.realname		
				FROM ##_m_surat a
				LEFT JOIN ##_m_jns_surat c ON a.jns_surat=c.id
    ...
    public function service($token, $servicename) {
		...
		switch($servicename) {
			...
			case 'listjnssurat' : $out = $this->listjnssurat(); break;
	...
	private function checkvar($d) {
		...
		$out = array(
			'id_files' => $d['idfile'],			
			'jns_surat' => $d['jns_surat'],
			'no_surat' => $d['no_surat'],
			'perihal' => $d['perihal'],
			'tglsurat' => $d['tglsurat'],
			'desksurat' => $d['desksurat'],
			'tglsurat' =>  $this->innerapp->tanggal->settgl($d['tglsurat']),
		);
		return $out;
	}
	private function listjnssurat() {
		$innerapp= $this->innerapp;
		$sql = $innerapp->appdb->results("select a.id, a.nm_jnssurat FROM ##_m_jns_surat a WHERE a.isactive=1");
		return $sql;
	}	
	private function saveadd() {
		...
		$sv = $innerapp->appdb->insert('##_m_surat', $items);
		if (!$sv) {
	...
	private function saveedit() {
		...
		$q = $innerapp->appdb->update('##_m_surat', $items, array('id' => $id));
		if (!$q) {
	...
	private function savedelete() {
		...
		$q = $innerapp->appdb->update('##_m_surat', $innerapp->stampdelete(), array('id' => $id));
		if (!$q) {
