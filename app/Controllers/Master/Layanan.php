...
{
	public $suburl = 'master/layanan';
	public $masksalt = '4m22';
	function __construct() {
		parent::__construct();
		
		$this->modulename = 'Layanan Akademik';
	...
	public function index()
    {
		...
		$data = array();
		$this->addJs('modules/master/layanan.js');
        return $this->ViewDefault($this->suburl.'/pagebody', $data);
    }
    private function grid() {
		$innerapp = $this->innerapp; 
		$filter = "";
		$searchcolumns = array( 'id' => 'a.id', 'jdl_layanan' => 'a.jdl_layanan');
		$displaycolumns = array('id','jdl_layanan','desklayanan');
		$sql = "SELECT a.id, a.jdl_layanan, a.desklayanan, a.datecreate, a.jnsfile, a.ketlayanan, a.id_files,
	...
	private function checkvar($d) {
		...
		$out = array(
			'id_files' => $d['idfile'],
			'jdl_layanan' => $d['jdllayanan'],
			'desklayanan' => $d['desklayanan'],
			'jnsfile' => $d['jnsfile'],
			'ketlayanan' => $d['ketlayanan'],
	...
	private function saveadd() {
		...
		$sv = $innerapp->appdb->insert('##_m_layanan', $items);
		if (!$sv) {
	...
	private function saveedit() {
		...
		$q = $innerapp->appdb->update('##_m_layanan', $items, array('id' => $id));
		if (!$q) {
	...
	private function savedelete() {
		...
		$q = $innerapp->appdb->update('##_m_layanan', $innerapp->stampdelete(), array('id' => $id));
		if (!$q) {
