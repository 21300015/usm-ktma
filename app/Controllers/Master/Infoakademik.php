...
class Infoakademik extends BaseController
{
	public $suburl = 'master/infoakademik';
	public $masksalt = '4m21';
	function __construct() {
		parent::__construct();
		
		$this->modulename = 'Info Akademik';
	...
	public function index()
    {
		...
		$data = array();
		$this->addJs('modules/master/infoakademik.js');
    ...
    private function grid() {
		...
		$sql = "SELECT a.id, a.jdl_info, a.deskinfo, a.datecreate, a.ketinfo, a.id_files,
		date_format(a.datecreate, '%d-%m-%Y') cd, b.realname 
		 FROM ##_m_infoakademik a
	...
	private function saveadd() {
		...
		$sv = $innerapp->appdb->insert('##_m_infoakademik', $items);
		if (!$sv) {
	...
	private function saveedit() {
		...
		$q = $innerapp->appdb->update('##_m_infoakademik', $items, array('id' => $id));
		if (!$q) {
	...
	private function savedelete() {
		...
		$q = $innerapp->appdb->update('##_m_infoakademik', $innerapp->stampdelete(), array('id' => $id));
		if (!$q) {
