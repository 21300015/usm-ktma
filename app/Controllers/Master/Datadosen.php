...
{
	public $suburl = 'master/datadosen';
	public $masksalt = '4m32';
	function __construct() {
		parent::__construct();
		
		$this->modulename = 'Data Dosen';
	...
	public function index()
    {
		...
		$data = array();
		$this->addJs('modules/master/datadosen.js');
        return $this->ViewDefault($this->suburl.'/pagebody', $data);
    }
    private function grid() {
		$innerapp = $this->innerapp; 
		$filter = "";
		$searchcolumns = array( 'id' => 'a.id', 'nip' => 'nip', 'namadosen' => 'namadosen');
		$displaycolumns = array('id','nip','nbm','namadosen','gender','alamat','email','nohp','prodi');
		$sql = "SELECT a.id, a.nip, a.nbm, a.namadosen, a.prodi, c.nm_prodi, a.gender, d.nm_gender, a.tempatlahir, a.tgllahir, CONCAT (a.tempatlahir,', ', a.tgllahir) AS ttl, a.alamat, a.nohp, a.email,
	...
	private function saveadd() {
		...
		$sv = $innerapp->appdb->insert('##_m_dosen', $items);
		if (!$sv) {
	...
	private function saveedit() {
		...
		$q = $innerapp->appdb->update('##_m_dosen', $items, array('id' => $id));
		if (!$q) {
	...
	private function savedelete() {
		...
		$q = $innerapp->appdb->update('##_m_dosen', $innerapp->stampdelete(), array('id' => $id));
		if (!$q) {
