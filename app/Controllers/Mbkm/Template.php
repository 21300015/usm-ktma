...
{
	public $suburl = 'mbkm/template';
	public $masksalt = '4mbkm23';
	function __construct() {
		parent::__construct();
		
		$this->modulename = 'Template MBKM';
	...
	public function index()
    {
		...
		$data = array();
		$this->addJs('modules/mbkm/template.js');
	...
	private function saveadd() {
		...
		$sv = $innerapp->appdb->insert('##_mbkm_template', $items);
		if (!$sv) {
	...
	private function saveedit() {
		...
		$q = $innerapp->appdb->update('##_mbkm_template', $items, array('id' => $id));
		if (!$q) {
	...
	private function savedelete() {
		...
		$q = $innerapp->appdb->update('##_mbkm_template', $innerapp->stampdelete(), array('id' => $id));
		if (!$q) {
