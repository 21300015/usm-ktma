...
class Monitoringsidang extends BaseController
{
	public $suburl = 'skripsi/monitoringsidang';
	public $masksalt = '4s25';
	function __construct() {
		parent::__construct();
		
		$this->modulename = 'Monitoring Sidang';
	...
	public function index()
    {
		...
		$data = array();
		$this->addJs('modules/skripsi/monitoringsidang.js');
    ...
    private function grid() {
		...
		$sql = "SELECT a.id, a.nim, a.namamhs, a.prodi, c.nm_prodi, a.jdl_skripsi, a.dospem, a.sempro, a.sidang, a.revisi, a.nilaiskripsi, a.id_files
		 FROM ##_m_monitoringsidang a
	...
	private function saveadd() {
		...
		$sv = $innerapp->appdb->insert('##_m_monitoringsidang', $items);
		if (!$sv) {
	...
	private function saveedit() {
		...
		$q = $innerapp->appdb->update('##_m_monitoringsidang', $items, array('id' => $id));
		if (!$q) {
	...
	private function savedelete() {
		...
		$q = $innerapp->appdb->update('##_m_monitoringsidang', $innerapp->stampdelete(), array('id' => $id));
		if (!$q) {
