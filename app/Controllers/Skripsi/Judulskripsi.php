...
class Judulskripsi extends BaseController
{
	public $suburl = 'skripsi/judulskripsi';
	public $masksalt = '4s22';
	function __construct() {
		parent::__construct();
		
		$this->modulename = 'Judul Skripsi/Tugas Akhir';
	...
	public function index()
    {
		...
		$data = array();
		$this->addJs('modules/skripsi/judulskripsi.js');
        return $this->ViewDefault($this->suburl.'/pagebody', $data);
    }
    private function grid() {
		$innerapp = $this->innerapp; 
		$filter = "";
		$searchcolumns = array( 'id' => 'a.id', 'jdlskripsi' => 'a.jdlskripsi', 'a.namamhs' => 'a.namamhs');
		$displaycolumns = array('id','jdlskripsi','namamhs');
		$sql = "SELECT a.id, a.nim, a.namamhs, a.prodi, a.jdlskripsi, a.dospem, a.id_files
		 FROM ##_s_judulskripsi a
	...
	private function saveadd() {
		...
		$sv = $innerapp->appdb->insert('##_s_judulskripsi', $items);
		if (!$sv) {
	...
	private function saveedit() {
		...
		$q = $innerapp->appdb->update('##_s_judulskripsi', $items, array('id' => $id));
		if (!$q) {
	...
	private function savedelete() {
		...
		$q = $innerapp->appdb->update('##_s_judulskripsi', $innerapp->stampdelete(), array('id' => $id));
		if (!$q) {
