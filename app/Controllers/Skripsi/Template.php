...
{
	public $suburl = 'skripsi/template';
	public $masksalt = '4s21';
	function __construct() {
		parent::__construct();
		
		$this->modulename = 'Template Skripsi';
		$this->trigger['add'] = $this->innerapp->servervar->encpt('Ra7ePA9sSSQ');
		$this->trigger['edit'] = $this->innerapp->servervar->encpt('Dn8Aue8CR7');
		$this->trigger['delete'] = $this->innerapp->servervar->encpt('Csix0suSE5u8');
		$this->trigger['upload'] = $this->innerapp->servervar->encpt('Csdu8kaD3bf');
	}
	public function index()
    {
		...
		$data = array();
		$this->addJs('modules/skripsi/template.js');
        return $this->ViewDefault($this->suburl.'/pagebody', $data);
    }
    private function grid() {
		$innerapp = $this->innerapp; 
		$filter = "";
		$searchcolumns = array( 'id' => 'a.id', 'jdl_template' => 'a.jdl_template');
		$displaycolumns = array('id','jdl_template','desktemplate');
		$sql = "SELECT a.id, a.jdl_template, a.desktemplate, a.datecreate, a.jnsfile, a.kettemplate, a.id_files,
	...
	private function checkvar($d) {
		...
		$out = array(
			'id_files' => $d['idfile'],
			'jdl_template' => $d['jdltemplate'],
			'desktemplate' => $d['desktemplate'],
			'jnsfile' => $d['jnsfile'],
			'kettemplate' => $d['kettemplate'],
	...
	private function saveadd() {
		...
		$sv = $innerapp->appdb->insert('##_s_template', $items);
		if (!$sv) {
	...
	private function saveedit() {
		...
		$q = $innerapp->appdb->update('##_s_template', $items, array('id' => $id));
		if (!$q) {
	...
	private function savedelete() {
		...
		$q = $innerapp->appdb->update('##_s_template', $innerapp->stampdelete(), array('id' => $id));
		if (!$q) {
